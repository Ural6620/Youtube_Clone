<template>
  <div class="relative -mt-1 ml-auto">
    ...
    >
      ...
      >
        <section class="py-1">
          <VideoItemDropdownListItem label="Add to queu" icon="menuAlt3" />
        </section>
      </div>
    </transition>
  </div>
...
export default {
  ...
  methods: {
    ...
    getBottomClass(event) {
      const clickCoordY = event.clientY;
      const dropdownHeght = this.$refs.dropdown.offsetHeight;

      if (window.innerHeight - clickCoordY < dropdownHeght) {
        return "bottom-9";
      }
